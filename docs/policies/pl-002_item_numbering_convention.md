...
3. Specific item types **may** carry a type-prefix before the numeric prefix. When a type-prefix is used, the filename form is `<type>-NNN_<rest>.md`. The type-prefix is a short, lowercase, snake_case identifier.
4. Type-prefixes currently authorized (update this list when adding or removing a type-prefix):
   - **Change requests** — `cr-` is **required**. Example: `cr-001_vector_search_mvp.md`.
   - **Policies** — `pl-` is **required** for every file under `docs/policies/`, including files nested in `docs/policies/domain_policies/`. Example: `pl-001_file_and_folder_naming_convention.md`. See `docs/policies/domain_policies/pl-001_all_policy_filenames_use_pl_prefix.md`.
...
- `pl-001_file_and_folder_naming_convention.md` (policy — `pl-` required)
- `pl-002_item_numbering_convention.md` (policy — `pl-` required)
- `cr-001_vector_search_mvp.md` (change request — `cr-` required)
