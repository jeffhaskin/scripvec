...
- The vision (`docs/specs/vision_tree/000_overall_vision.md`) and MVP CR (`docs/specs/change_requests/drafting/cr_001_vector_search_mvp.md`) describe a system with distinct bounded contexts: corpus ingest, citation parsing, retrieval (embeddings + index + BM25 + hybrid), and evaluation. Those are the MVP's domains.
...
- Folder names use underscores, lowercase, per policy `001_file_and_folder_naming_convention.md`.
- Packages are **peer modules** with no inherent sequence; policy `002_item_numbering_convention.md` governs ordered documents, not package folders. **Packages do not carry numeric prefixes.**
...
- `docs/policies/001_file_and_folder_naming_convention.md` — underscore rule applied to folder names here.
- `docs/policies/002_item_numbering_convention.md` — referenced in the *packages carry no numeric prefix* clause.
...
- `docs/specs/change_requests/drafting/cr_001_vector_search_mvp.md` — MVP scope; every in-scope item above has a home in this structure.
